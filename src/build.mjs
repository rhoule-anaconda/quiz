...
// Bundle all quizzes for the Worker (multiplayer mode reads them server-side
// so answer keys never reach players' browsers).
const WORKER_GEN_DIR = path.join(ROOT, "src", "worker", "gen");
fs.mkdirSync(WORKER_GEN_DIR, { recursive: true });
const bundle = {};
for (const file of files) {
  const slug = path.basename(file, ".json");
  bundle[slug] = JSON.parse(fs.readFileSync(path.join(QUESTIONS_DIR, file), "utf8"));
}
const bundleSrc = `// Generated by src/build.mjs — do not edit
import type { Quiz } from "../game-room.ts";
export const QUIZZES: Record<string, Quiz> = ${JSON.stringify(bundle, null, 2)};
export const QUIZ_LIST: { slug: string; title: string; count: number }[] = ${
  JSON.stringify(
    built.map(b => ({ slug: b.slug, title: b.title, count: b.count })),
    null,
    2
  )
};
`;
fs.writeFileSync(path.join(WORKER_GEN_DIR, "quizzes.ts"), bundleSrc);
